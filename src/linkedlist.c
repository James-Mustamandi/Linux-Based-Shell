...
/*
    ...
    -------------------------------                                         
*/
int BgComparator(const void* lhs, const void* rhs) {
    const bgentry_t* bgLHS = (const bgentry_t*) lhs;
    const bgentry_t* bgRHS = (const bgentry_t*) rhs;

    if(bgLHS->seconds < bgRHS->seconds) {
        return -1;
    }
    else if(bgLHS->seconds > bgRHS->seconds) {
        return 1;
    }
    else {
        return 0;
...
void BgPrinter(void* data, void* a) {
    bgentry_t* bg = (bgentry_t*) data;
    print_bgentry(bg);

}

void DeleteBgProcess(void* data) {
    bgentry_t* bg = (bgentry_t*) data;
    free(bg);
}
...
bgentry_t* FindEntryByPID(list_t* list, pid_t pid) {
     if(list == NULL) {
        return NULL;
    }
    node_t* p = list->head;
    while(p != NULL) {
        if(((bgentry_t*)p->data)->pid == pid) {
        return (bgentry_t*)p->data;
        }
        p = p->next;
    }
    return NULL;
...
int RemoveJobFromList(list_t* list, pid_t pid) {
    if(list == NULL) {
        return 0;
    }
    // printf("Trying to remove pid: %d\n", pid);
    int i = 0;
    node_t* p = list->head;
    while(p != NULL) {
        if(((bgentry_t*)p->data)->pid == pid) {
